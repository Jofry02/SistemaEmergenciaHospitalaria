...
export default function ConsultaPage() {
  ...
  const consultasPrevias = selectedPaciente
    ? consultasDelDia.filter((c) => {
        const ced = c.pacienteCedula || c.PacienteCedula || c.cedula || c.Cedula || ''
  ...
  async function handleEliminarConsulta(consultaId, e) {
    e.stopPropagation()
    if (!window.confirm('¿Está seguro de eliminar esta consulta?')) return
    setDeletingId(consultaId)
    try {
      const res = await DataService.eliminarConsulta(consultaId)
      if (res?.success) {
        cargarConsultas()
      } else {
        setApiError(res?.error || 'Error al eliminar consulta')
      }
    } catch (err) {
      setApiError(err?.message || 'Error de conexión con el backend')
    } finally {
                      ...
                      >
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ fontSize: '0.7rem', color: '#94a3b8', marginTop: '2px' }}>
                            {c.fechaHoraLlegada || c.FechaHoraLlegada || c.horaLlegada || ''}
                          </div>
                        </div>
                        <button
                          onClick={(e) => handleEliminarConsulta(c.consultaID || c.ConsultaID || c.id, e)}
                          style={{
                            background: 'none',
                            border: 'none',
                            color: '#94a3b8',
                        ...
                        >
                          {deletingId === (c.consultaID || c.ConsultaID || c.id) ? (
                  ...
                  <div className="modal__section">
                    ...
                    <div className="modal__vitals-grid">
                      ...
                      <div className="modal__vital-item">
                        ...
                        <span className="modal__vital-value">{detalleConsulta.saturacionOxigeno || detalleConsulta.SaturacionOxigeno || detalleConsulta.saturacionO2 || '—'} <small>%</small></span>
                      </div>
                    </div>
                  </div>
                </div>
