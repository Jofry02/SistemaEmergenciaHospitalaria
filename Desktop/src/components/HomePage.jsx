...
export default function HomePage() {
  ...
  const enEspera = consultas.filter((c) => {
    const estado = (c.estado || c.Estado || c.status || '').toLowerCase()
                    ...
                    <div className="home__column-list">
                      {items.length === 0 ? (
                        ...
                        items.map((c, idx) => {
                          ...
                          const nombrePac = c.pacienteNombre || c.PacienteNombre || c.nombreCompleto || c.NombreCompleto || ''
                          const motivo = c.motivoConsulta || c.MotivoConsulta || ''
                          const fechaHora = c.fechaHoraLlegada || c.FechaHoraLlegada || c.horaLlegada || ''
                          return (
