...
- **`validate()`**: valida todos los campos (datos personales + clínicos) — usado internamente
- **`validatePersonalData()`**: valida solo datos personales (nombre, cédula, fecha, teléfono, tipo sangre, sexo) — usado en RegistroPage
- **`validateConsulta()`**: valida consulta clínica — motivo, hora, signos vitales condicionales con rangos numéricos (FC 0-300, FR 0-100, Temp 30-45°C, SpO2 0-100%, Peso 0-500kg, Talla 0-3m)
- **`resetForm()`**: reinicia todo el formulario
- **`loadPatient(paciente, prevFormData)`**: carga datos de un paciente existente (soporta camelCase, PascalCase y nombres de campos mock)
...
El puente entre JavaScript y C#. Usa `DataTableToList` para convertir `DataTable` a `List<Dictionary>` y `JsonSerializer` con `CamelCase` para que los nombres de campo coincidan con el frontend React. Todos los métodos de consulta devuelven `{ success: true, data: [...] }`.
...
public class BackendBridge
{
    ...
    private string Json(object obj) => JsonSerializer.Serialize(obj, _jsonOpts);
    private string Ok(object? extra = null) =>
        extra != null ? Json(new { success = true, data = extra }) : Json(new { success = true });
    private string Error(Exception ex) => Json(new { success = false, error = ex.Message });
}
```
