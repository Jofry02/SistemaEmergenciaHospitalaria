...
public class BackendBridge
{
    ...
    public string ActualizarPaciente(string json)
    {
        try
        {
            var data = JsonDocument.Parse(json).RootElement;
            var p = new Paciente
            {
                PacienteID = GetInt(data, "pacienteId") ?? 0,
                Cedula = GetString(data, "cedula") ?? "",
                NombreCompleto = GetString(data, "nombre") ?? "",
                FechaNacimiento = ParseDate(GetString(data, "fechaNacimiento")),
                Sexo = GetString(data, "sexo"),
            ...
            var filas = _pacienteBll.ActualizarPaciente(p);
            return Ok(new { success = filas > 0, pacienteId = p.PacienteID });
        }
        catch (Exception ex)
        {
            return Error(ex);
    ...
    public string EliminarPaciente(int pacienteId)
    {
        try
        {
            var ok = _pacienteBll.EliminarPaciente(pacienteId);
    ...
    public string EliminarConsulta(int consultaId)
    {
        try
        {
            var ok = _pacienteBll.EliminarConsulta(consultaId);
            return Ok(new { success = ok });
        }
        catch (Exception ex)
        {
    ...
    public string RegistrarConsulta(string json)
    {
        try
        {
            ...
            var c = new Consulta
            {
                ...
                PresionArterial = CleanPresionArterial(GetString(data, "presionArterial")),
    ...
    private static string? CleanPresionArterial(string? val)
    {
    ...
    private static int? GetInt(JsonElement data, string key)
    {
        if (data.TryGetProperty(key, out var prop) && prop.ValueKind == JsonValueKind.Number)
